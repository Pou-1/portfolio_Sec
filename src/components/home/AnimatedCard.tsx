...
const AnimatedCard: React.FC<AnimatedCardProps> = ({ image, text, logos, scrollY, scrollYMin, scrollYMax }) => {
  return (
    <div
    ...
    >
      ...
      <div className="pt-4 w-full flex justify-between z-10 text-lg">
        <p>{text}</p>
        <div className="flex gap-2">{logos.map((logo, index) => <span key={index}>{logo}</span>)}</div>
      </div>
    </div>
  );
};
