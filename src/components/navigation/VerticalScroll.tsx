...
const VerticalScroll: React.FC<VerticalScrollProps> = ({
	...
	useEffect(() => {
		...
		if (firstButton) {
			...
			const parentRect =
				firstButton.offsetParent!.getBoundingClientRect();
			setBlueBoxStyle({
	...
	useEffect(() => {
		...
		if (targetButton && blueBoxRef.current) {
			...
			const parentRect =
				targetButton.offsetParent!.getBoundingClientRect();
	...
	const handleButtonClick = (index: number) => {
		...
		if (currentButton && targetButton) {
			...
			const parentRect =
				currentButton.offsetParent!.getBoundingClientRect();
	...
	return (
		<div className="relative z-50 flex-center flex-col">
			{/* rond animé */}
			<div
				className="absolute z-[110] bg-customGreen rounded-full trans-fast -translate-x-2.5"
				style={{
            ...
            dark:hover:bg-customDarkLight trans-fast rounded-full border-transparent"
					...
					>
						<p className="h-fit py-2 flex-center w-32 px-4">
							{section}
						</p>
						<button
							ref={(el) => (buttonRefs.current[index] = el!)}
							className="h-5 aspect-square rounded-full trans-fast group-hover:scale-110 bg-customGreen/20 hover:bg-customGreen/70"
						/>
					</div>
				))}
			</div>
			...
			<div className="flex-center relative flex-col gap-y-10 -translate-x-2.5">
				{[0, 1, 2, 3, 4].map((index) => (
					<button
						key={index + "b"}
						ref={(el) => (buttonRefs.current[index] = el!)}
						className="w-5 h-5 rounded-full trans-fast hover:scale-110 hover:bg-customGreen/70"
						onClick={() => handleButtonClick(index)}
					/>
				))}
			</div>
